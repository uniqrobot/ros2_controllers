...
namespace
{
constexpr auto DEFAULT_COMMAND_TOPIC = "~/cmd_vel";//"~/vel_to_velStamped";//"~/cmd_vel";
  ...
  const std::string & controller_name)
{
  ...
  lifecycle_node_->declare_parameter<bool>("enable_imu", true);
  imu_sub= lifecycle_node_->create_subscription<sensor_msgs::msg::Imu>("imu", 10, std::bind(&DiffDriveController::imuCallback, this, std::placeholders::_1));
...
CallbackReturn DiffDriveController::on_configure(const rclcpp_lifecycle::State &)
{
  ...
  enable_imu = lifecycle_node_->get_parameter(
    "enable_imu").as_bool();
  const auto wheels = wheel_params_;
